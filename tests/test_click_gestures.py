...
def test_double_click_candidate_suppresses_left_click() -> None:
    ...
    activated = item.update(0.2, 0.2, 0.03)
    assert activated.action is ClickAction.DOUBLE_CLICK


def test_tracking_loss_reset_clears_both_recognizers() -> None:
    item = coordinator()
    item.update(0.2, 0.8, 0.0)
    item.reset(0.01)
    update = item.update(0.2, 0.2, 0.02)
